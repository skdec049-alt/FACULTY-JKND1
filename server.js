...
const path = require('path'); // ADDED: Built-in node module to handle file paths smoothly
// Only one import at the top for everything from database.js
...
// ==========================================================
// This tells Express to automatically serve your HTML, CSS, and JS frontend pages
app.use(express.static(path.join(__dirname)));

// Route to serve your main Index.html page when visiting the base URL
app.get('/', (req, res) => {
...
app.post('/api/signup', async (req, res) => {
    try {
        // Ensure college and designation are read from the incoming body
        const { fullname, email, college, department, designation, password } = req.body;
        
        const newFaculty = await Faculty.create({
            fullname,
            email,
            college,       // <-- Verify this line exists!
            department,
            designation,   // <-- Verify this line exists!
            password
        });
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        // CHANGED: Returning college and designation profiles to save cleanly inside localStorage
        res.json({ 
            message: "Login successful!", 
            user: { 
                id: faculty.id, 
                fullname: faculty.fullname, 
                college: faculty.college,             // CHANGED: Shared to client side
                department: faculty.department,
                designation: faculty.designation,     // CHANGED: Shared to client side
                score: faculty.score
            } 
        });
    } catch (error) {
...
app.get('/api/faculty', async (req, res) => {
    try {
        // CHANGED: Appended 'college' and 'designation' database column targets to map summary tables smoothly
...
app.get('/api/admin/reports/all', async (req, res) => {
    try {
        // Fetch all submissions from the database and include the related Faculty table data
        ...
        // Remap the database entries to match the variable format used by admin_dashboard.html
...
// CHANGED: Uses process.env.PORT for Render environment compatibility
const PORT = process.env.PORT || 3000;

// Force synchronization step safely on database connection setup
sequelize.sync().then(() => {
    app.listen(PORT, () => {
